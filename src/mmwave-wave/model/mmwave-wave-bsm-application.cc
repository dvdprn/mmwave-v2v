...
#include "ns3/mmwave-wave-bsm-application.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("MmWaveWaveBsmApplication");

namespace ns3 {

// (Arbitrary) port for establishing socket to transmit WAVE BSMs
int MmWaveWaveBsmApplication::wavePort = 9080;

NS_OBJECT_ENSURE_REGISTERED (MmWaveWaveBsmApplication);

TypeId
MmWaveWaveBsmApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWaveWaveBsmApplication")
    .SetParent<Application> ()
    .SetGroupName ("MmWave-Wave")
    .AddConstructor<MmWaveWaveBsmApplication> ()
    ;
  return tid;
}

MmWaveWaveBsmApplication::MmWaveWaveBsmApplication ()
  : m_waveBsmStats (0),
    m_txSafetyRangesSq (),
...
void
MmWaveWaveBsmApplication::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
// Application Methods
void MmWaveWaveBsmApplication::StartApplication () // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
  ...
  recvSink->SetRecvCallback (MakeCallback (&MmWaveWaveBsmApplication::ReceiveWavePacket, this));
  ...
  Simulator::ScheduleWithContext (recvSink->GetNode ()->GetId (),
                                  txTime, &MmWaveWaveBsmApplication::GenerateWaveTraffic, this,
                                  recvSink, m_wavePacketSize, m_numWavePackets, waveInterPacketInterval, m_nodeId);
}

void MmWaveWaveBsmApplication::StopApplication () // Called at time specified by Stop
{
  NS_LOG_FUNCTION (this);
}

void
MmWaveWaveBsmApplication::Setup (Ipv4InterfaceContainer & i,
                       int nodeId,
                       Time totalTime,
                       ...
                       Ptr<MmWaveWaveBsmStats> waveBsmStats,
...
void
MmWaveWaveBsmApplication::GenerateWaveTraffic (Ptr<Socket> socket, uint32_t pktSize,
...
void MmWaveWaveBsmApplication::ReceiveWavePacket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this);
...
void MmWaveWaveBsmApplication::HandleReceivedBsmPacket (Ptr<Node> txNode,
                                              Ptr<Node> rxNode)
{
  NS_LOG_FUNCTION (this);
...
int64_t
MmWaveWaveBsmApplication::AssignStreams (int64_t streamIndex)
{
  NS_LOG_FUNCTION (this);
...
Ptr<Node>
MmWaveWaveBsmApplication::GetNode (int id)
{
  NS_LOG_FUNCTION (this);
...
Ptr<NetDevice>
MmWaveWaveBsmApplication::GetNetDevice (int id)
{
  NS_LOG_FUNCTION (this);
