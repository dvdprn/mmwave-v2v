...
NS_LOG_COMPONENT_DEFINE ("MmWaveWaveMacLow");

NS_OBJECT_ENSURE_REGISTERED (MmWaveWaveMacLow);

TypeId
MmWaveWaveMacLow::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWaveWaveMacLow")
    .SetParent<MacLow> ()
    .SetGroupName ("MmWave-Wave")
    .AddConstructor<MmWaveWaveMacLow> ()
  ;
  return tid;
}
MmWaveWaveMacLow::MmWaveWaveMacLow ()
{
...
void
MmWaveWaveMacLow::SetMmWaveWaveNetDevice (Ptr<MmWaveWaveNetDevice> device)
{
  m_scheduler  =  device->GetMmWaveWaveChannelScheduler ();
  m_coordinator =  device->GetMmWaveWaveChannelCoordinator ();
  NS_ASSERT (m_scheduler != 0 && m_coordinator != 0);
}

WifiTxVector
MmWaveWaveMacLow::GetDataTxVector (Ptr<const Packet> packet, const WifiMacHeader *hdr) const
{
  NS_LOG_FUNCTION (this << packet << hdr);
  MmWaveWaveHigherLayerTxVectorTag datatag;
  bool found;
...
void
MmWaveWaveMacLow::StartTransmission (Ptr<const Packet> packet,
