 ...
 */
#ifndef MMWAVE_WAVE_NET_DEVICE_H
#define MMWAVE_WAVE_NET_DEVICE_H
...
#include "mmwave-wave-vsa-manager.h"
namespace ns3 {
struct SchInfo;
struct VsaInfo;
class NetDevice;
class MmWaveWaveChannelScheduler;
class MmWaveWaveVsaManager;
class MmWaveWaveOcbWifiMac;
 ...
 * and ns3::MmWaveWaveOcbWifiMac (including ns3::WifiRemoteStationManager).
 * Besides that, to support multiple channel operation this
 * class also holds ns3::MmWaveWaveChannelScheduler, ns3::MmWaveWaveChannelManager,
 * ns3::MmWaveWaveChannelCoordinator and ns3::MmWaveWaveVsaManager.
 *
 ...
 * void SetUtcTime ();
 */
class MmWaveWaveNetDevice : public NetDevice
{
public:
  /**
  ...
  MmWaveWaveNetDevice (void);
  virtual ~MmWaveWaveNetDevice (void);

  /**
   * \param channelNumber the specific channel
   * \param mac add a new available MAC entity for specific channel
   */
  void AddMac (uint32_t channelNumber, Ptr<MmWaveWaveOcbWifiMac> mac);
  /**
   * \param channelNumber the specific channel number
   * \return corresponding MAC entity
   */
  Ptr<MmWaveWaveOcbWifiMac> GetMac (uint32_t channelNumber) const;
  /**
   * \return all inserted MAC entities.
   */
  std::map<uint32_t, Ptr<MmWaveWaveOcbWifiMac> >  GetMacs (void) const;
  /**
   ...
   */
  void SetMmWaveWaveChannelScheduler (Ptr<MmWaveWaveChannelScheduler> channelScheduler);
  /**
   * \return current channel scheduler for multiple channel operation
   */
  Ptr<MmWaveWaveChannelScheduler> GetMmWaveWaveChannelScheduler (void) const;
  /**
   * \param channelManager the channel manager for multiple channel operation
   */
  void SetMmWaveWaveChannelManager (Ptr<MmWaveWaveChannelManager> channelManager);
  /**
   * \return currentc channel manager for multiple channel operation
   */
  Ptr<MmWaveWaveChannelManager> GetMmWaveWaveChannelManager (void) const;
  /**
   * \param channelCoordinator  the channel coordinator for multiple channel operation
   */
  void SetMmWaveWaveChannelCoordinator (Ptr<MmWaveWaveChannelCoordinator> channelCoordinator);
  /**
   * \return current channel coordinator for multiple channel operation
   */
  Ptr<MmWaveWaveChannelCoordinator> GetMmWaveWaveChannelCoordinator (void) const;
  /**
   * \param vsaManager the VSA manager for multiple channel operation
   */
  void SetMmWaveWaveVsaManager (Ptr<MmWaveWaveVsaManager> vsaManager);
  /**
   * \return current VSA manager for multiple channel operation
   */
  Ptr<MmWaveWaveVsaManager> GetMmWaveWaveVsaManager (void) const;
   ...
   *
   * note: This method should be called before MmWaveWaveNetDevice::Send method
   */
...
private:
  ...
  /// MacEntities typedef
  typedef std::map<uint32_t, Ptr<MmWaveWaveOcbWifiMac> > MacEntities;
  /// MacEntities iterator typedef
  typedef std::map<uint32_t, Ptr<MmWaveWaveOcbWifiMac> >::const_iterator MacEntitiesI;
  ...
  Ptr<MmWaveWaveChannelManager> m_channelManager; ///< the channel manager
  Ptr<MmWaveWaveChannelScheduler> m_channelScheduler; ///< the channel scheduler
  Ptr<MmWaveWaveChannelCoordinator> m_channelCoordinator; ///< the channel coordinator
  Ptr<MmWaveWaveVsaManager> m_vsaManager; ///< the VSA manager 
