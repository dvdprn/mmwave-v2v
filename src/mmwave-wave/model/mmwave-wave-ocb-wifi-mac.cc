...
#include "mmwave-wave-mac-low.h"
#include "mmwave-wave-ocb-wifi-mac.h"
#include "mmwave-wave-vendor-specific-action.h"
#include "mmwave-wave-higher-tx-tag.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("MmWaveWaveOcbWifiMac");

NS_OBJECT_ENSURE_REGISTERED (MmWaveWaveOcbWifiMac);

/// Wildcard BSSID
const static Mac48Address WILDCARD_BSSID = Mac48Address::GetBroadcast ();

TypeId
MmWaveWaveOcbWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWaveWaveOcbWifiMac")
    .SetParent<RegularWifiMac> ()
    .SetGroupName ("MmWave-Wave")
    .AddConstructor<MmWaveWaveOcbWifiMac> ()
  ;
  return tid;
}

MmWaveWaveOcbWifiMac::MmWaveWaveOcbWifiMac (void)
{
...
void
MmWaveWaveOcbWifiMac::SendVsc (Ptr<Packet> vsc, Mac48Address peer, OrganizationIdentifier oi)
{
  ...
  hdr.SetDsNotFrom ();
  hdr.SetDsNotTo ();
  MmWaveWaveVendorSpecificActionHeader vsa;
...
void
MmWaveWaveOcbWifiMac::AddReceiveVscCallback (OrganizationIdentifier oi, VscCallback cb)
{
...
void
MmWaveWaveOcbWifiMac::RemoveReceiveVscCallback (OrganizationIdentifier oi)
{
...
void
MmWaveWaveOcbWifiMac::SetSsid (Ssid ssid)
{
  NS_LOG_WARN ("in OCB mode we should not call SetSsid");
}

Ssid
MmWaveWaveOcbWifiMac::GetSsid (void) const
{
...
void
MmWaveWaveOcbWifiMac::SetBssid (Mac48Address bssid)
{
  NS_LOG_WARN ("in OCB mode we should not call SetBsid");
}

Mac48Address
MmWaveWaveOcbWifiMac::GetBssid (void) const
{
...
void
MmWaveWaveOcbWifiMac::SetLinkUpCallback (Callback<void> linkUp)
{
...
void
MmWaveWaveOcbWifiMac::SetLinkDownCallback (Callback<void> linkDown)
{
...
void
MmWaveWaveOcbWifiMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
 ...
 */
void
MmWaveWaveOcbWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  ...
  if (hdr->IsMgt () && hdr->IsAction ())
    {
      // yes, we only care about MmWaveWaveVendorSpecificAction frame in OCB mode
      // other management frames will be handled by RegularWifiMac::Receive
      MmWaveWaveVendorSpecificActionHeader vsaHdr;
...
void
MmWaveWaveOcbWifiMac::ConfigureEdca (uint32_t cwmin, uint32_t cwmax, uint32_t aifsn, enum AcIndex ac)
{
...
void
MmWaveWaveOcbWifiMac::FinishConfigureStandard (enum WifiPhyStandard standard)
{
  NS_LOG_FUNCTION (this << standard);
  NS_ASSERT ((standard == WIFI_PHY_STANDARD_80211_10MHZ)
             || (standard == WIFI_PHY_STANDARD_80211a)
             || (standard == WIFI_PHY_MMWAVE_80211_60GHZ));
...
void
MmWaveWaveOcbWifiMac::Suspend (void)
{
  NS_LOG_FUNCTION (this);
...
void
MmWaveWaveOcbWifiMac::Resume (void)
{
  NS_LOG_FUNCTION (this);
...
void
MmWaveWaveOcbWifiMac::MakeVirtualBusy (Time duration)
{
...
void
MmWaveWaveOcbWifiMac::CancleTx (enum AcIndex ac)
{
...
void
MmWaveWaveOcbWifiMac::Reset (void)
{
  NS_LOG_FUNCTION (this);
...
void
MmWaveWaveOcbWifiMac::EnableForWave (Ptr<MmWaveWaveNetDevice> device)
{
  NS_LOG_FUNCTION (this << device);
  // To extend current MmWaveWaveOcbWifiMac for WAVE 1609.4, we shall use MmWaveWaveMacLow instead of MacLow
  m_low = CreateObject<MmWaveWaveMacLow> ();
  (DynamicCast<MmWaveWaveMacLow> (m_low))->SetMmWaveWaveNetDevice (device);
