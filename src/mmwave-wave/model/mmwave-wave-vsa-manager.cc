...
NS_LOG_COMPONENT_DEFINE ("MmWaveWaveVsaManager");

NS_OBJECT_ENSURE_REGISTERED (MmWaveWaveVsaManager);
...
TypeId
MmWaveWaveVsaManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MmWaveWaveVsaManager")
    .SetParent<Object> ()
    .SetGroupName ("MmWave-Wave")
    .AddConstructor<MmWaveWaveVsaManager> ()
  ;
  return tid;
}

MmWaveWaveVsaManager::MmWaveWaveVsaManager (void)
  : m_device (0)
{
...
void
MmWaveWaveVsaManager::DoDispose (void)
{
...
void
MmWaveWaveVsaManager::DoInitialize (void)
{
  std::map<uint32_t, Ptr<MmWaveWaveOcbWifiMac> > macs = m_device->GetMacs ();
  for (std::map<uint32_t, Ptr<MmWaveWaveOcbWifiMac> >::iterator i = macs.begin (); i != macs.end (); ++i)
    {
      i->second->AddReceiveVscCallback (oi_1609, MakeCallback (&MmWaveWaveVsaManager::ReceiveVsc, this));
    }
}

void
MmWaveWaveVsaManager::SetMmWaveWaveNetDevice (Ptr<MmWaveWaveNetDevice> device)
{
...
void
MmWaveWaveVsaManager::SendVsa (const VsaInfo & vsaInfo)
{
  ...
  if (vsaInfo.peer.IsGroup () && (vsaInfo.repeatRate != 0))
    {
      ...
      vsa->vsc = vsaInfo.vsc;
      vsa->oi = oi;
      vsa->repeat =  Simulator::Schedule (vsa->repeatPeriod, &MmWaveWaveVsaManager::DoRepeat, this, vsa);
...
void
MmWaveWaveVsaManager::DoRepeat (VsaWork *vsa)
{
...
void
MmWaveWaveVsaManager::DoSendVsa (enum VsaTransmitInterval  interval, uint32_t channel,
                       Ptr<Packet> vsc, OrganizationIdentifier oi, Mac48Address peer)
{
  ...
  Ptr<MmWaveWaveChannelCoordinator> coordinator = m_device->GetMmWaveWaveChannelCoordinator ();
  Ptr<MmWaveWaveChannelScheduler> scheduler = m_device->GetMmWaveWaveChannelScheduler ();
  Ptr<MmWaveWaveChannelManager> manager = m_device->GetMmWaveWaveChannelManager ();
  ...
  if (interval == VSA_TRANSMIT_IN_SCHI)
    {
      ...
      if (wait != Seconds (0))
        {
          Simulator::Schedule (wait, &MmWaveWaveVsaManager::DoSendVsa, this,
  ...
  MmWaveWaveHigherLayerTxVectorTag tag = MmWaveWaveHigherLayerTxVectorTag (txVector, manager->GetManagementAdaptable (channel));
  vsc->AddPacketTag (tag);

  Ptr<MmWaveWaveOcbWifiMac> mac = m_device->GetMac (channel);
  mac->SendVsc (vsc, peer, oi);
}

void
MmWaveWaveVsaManager::RemoveAll (void)
{
  NS_LOG_FUNCTION (this);
...
void
MmWaveWaveVsaManager::RemoveByChannel (uint32_t channelNumber)
{
...
void
MmWaveWaveVsaManager::RemoveByOrganizationIdentifier (const OrganizationIdentifier &oi)
{
...
void
MmWaveWaveVsaManager::SetWaveVsaCallback (Callback<bool, Ptr<const Packet>,const Address &, uint32_t, uint32_t> vsaCallback)
{
  NS_LOG_FUNCTION (this);
...
bool
MmWaveWaveVsaManager::ReceiveVsc (Ptr<WifiMac> mac, const OrganizationIdentifier &oi, Ptr<const Packet> vsc, const Address &src)
{
