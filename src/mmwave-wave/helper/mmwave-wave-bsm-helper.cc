...
NS_LOG_COMPONENT_DEFINE ("MmWaveWaveBsmHelper");

namespace ns3 {

std::vector<int> MmWaveWaveBsmHelper::nodesMoving;

MmWaveWaveBsmHelper::MmWaveWaveBsmHelper ()
  : m_waveBsmStats ()
{
  ...
  m_factory.SetTypeId ("ns3::MmWaveWaveBsmApplication");
}

void
MmWaveWaveBsmHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
MmWaveWaveBsmHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
MmWaveWaveBsmHelper::Install (Ipv4InterfaceContainer i) const
{
...
Ptr<Application>
MmWaveWaveBsmHelper::InstallPriv (Ptr<Node> node) const
{
...
void
MmWaveWaveBsmHelper::Install (Ipv4InterfaceContainer & i,
                        ...
                        Time txMaxDelay)         // max delay prior to transmit
{
  ...
  for (aci = bsmApps.Begin (); aci != bsmApps.End (); ++aci)
    {
      Ptr<MmWaveWaveBsmApplication> bsmApp = DynamicCast<MmWaveWaveBsmApplication> (*aci);
      bsmApp->Setup (i,
                     nodeId,
                     totalTime,
                     wavePacketSize,
...
Ptr<MmWaveWaveBsmStats>
MmWaveWaveBsmHelper::GetMmWaveWaveBsmStats ()
{
  return &m_waveBsmStats;
}

int64_t
MmWaveWaveBsmHelper::AssignStreams (NodeContainer c, int64_t stream)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      for (uint32_t j = 0; j < node->GetNApplications (); j++)
        {
          Ptr<MmWaveWaveBsmApplication> bsmApp = DynamicCast<MmWaveWaveBsmApplication> (node->GetApplication (j));
          if (bsmApp)
            {
...
std::vector<int>&
MmWaveWaveBsmHelper::GetNodesMoving ()
{
  return nodesMoving;
}
