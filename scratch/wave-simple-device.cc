...
using namespace ns3;
/**
 * This simulation is to show the routing service of MmWaveWaveNetDevice described in IEEE 09.4.
 *
 ...
 */
class MmWaveWaveNetDeviceExample
{
public:
...
void
MmWaveWaveNetDeviceExample::CreateWaveNodes (void)
{
  nodes = NodeContainer ();
  nodes.Create (2);
  ...
  for (uint32_t i = 0; i != devices.GetN (); ++i)
    {
      Ptr<MmWaveWaveNetDevice> device = DynamicCast<MmWaveWaveNetDevice> (devices.Get (i));
      device->SetReceiveCallback (MakeCallback (&MmWaveWaveNetDeviceExample::Receive, this));
      device->SetWaveVsaCallback (MakeCallback  (&MmWaveWaveNetDeviceExample::ReceiveVsa, this));
    }
...
bool
MmWaveWaveNetDeviceExample::Receive (Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode, const Address &sender)
{
  SeqTsHeader seqTs;
...
void
MmWaveWaveNetDeviceExample::SendOneWsmpPacket  (uint32_t channel, uint32_t seq)
{
  Ptr<MmWaveWaveNetDevice>  sender = DynamicCast<MmWaveWaveNetDevice> (devices.Get (0));
  Ptr<MmWaveWaveNetDevice>  receiver = DynamicCast<MmWaveWaveNetDevice> (devices.Get (1));
...
void
MmWaveWaveNetDeviceExample::SendWsmpExample ()
{
  CreateWaveNodes ();
  Ptr<MmWaveWaveNetDevice>  sender = DynamicCast<MmWaveWaveNetDevice> (devices.Get (0));
  Ptr<MmWaveWaveNetDevice>  receiver = DynamicCast<MmWaveWaveNetDevice> (devices.Get (1));
  ...
  Simulator::Schedule (Seconds (0.0), &MmWaveWaveNetDevice::StartSch,sender,schInfo);
  // An important point is that the receiver should also be assigned channel
  // access for the same channel to receive packets.
  Simulator::Schedule (Seconds (0.0), &MmWaveWaveNetDevice::StartSch, receiver, schInfo);

  // send WSMP packets
  // the first packet will be queued currently and be transmitted in next SCH interval
  Simulator::Schedule (Seconds (1.0), &MmWaveWaveNetDeviceExample::SendOneWsmpPacket,  this, SCH1, 1);
  // the second packet will be queued currently and then be transmitted , because of in the CCH interval.
  Simulator::Schedule (Seconds (1.0), &MmWaveWaveNetDeviceExample::SendOneWsmpPacket,  this, CCH, 2);
  // the third packet will be dropped because of no channel access for SCH2.
  Simulator::Schedule (Seconds (1.0), &MmWaveWaveNetDeviceExample::SendOneWsmpPacket,  this, SCH2, 3);

  // release SCH access
  Simulator::Schedule (Seconds (2.0), &MmWaveWaveNetDevice::StopSch, sender, SCH1);
  Simulator::Schedule (Seconds (2.0), &MmWaveWaveNetDevice::StopSch, receiver, SCH1);
  // the fourth packet will be queued and be transmitted because of default CCH access assigned automatically.
  Simulator::Schedule (Seconds (3.0), &MmWaveWaveNetDeviceExample::SendOneWsmpPacket,  this, CCH, 4);
  // the fifth packet will be dropped because of no SCH1 access assigned
  Simulator::Schedule (Seconds (3.0), &MmWaveWaveNetDeviceExample::SendOneWsmpPacket,  this, SCH1, 5);
...
void
MmWaveWaveNetDeviceExample::SendIpPacket (uint32_t seq, bool ipv6)
{
  Ptr<MmWaveWaveNetDevice>  sender = DynamicCast<MmWaveWaveNetDevice> (devices.Get (0));
  Ptr<MmWaveWaveNetDevice>  receiver = DynamicCast<MmWaveWaveNetDevice> (devices.Get (1));
...
void
MmWaveWaveNetDeviceExample::SendIpExample ()
{
  CreateWaveNodes ();
  Ptr<MmWaveWaveNetDevice>  sender = DynamicCast<MmWaveWaveNetDevice> (devices.Get (0));
  Ptr<MmWaveWaveNetDevice>  receiver = DynamicCast<MmWaveWaveNetDevice> (devices.Get (1));
  ...
  Simulator::Schedule (Seconds (1.0), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 1, true);
  Simulator::Schedule (Seconds (1.050), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 2, false);
  //register txprofile
  // IP packets will automatically be sent with txprofile parameter
  const TxProfile txProfile = TxProfile (SCH1);
  Simulator::Schedule (Seconds (2.0), &MmWaveWaveNetDevice::RegisterTxProfile, sender, txProfile);
  // both IPv4 and IPv6 packet are transmitted successfully
  Simulator::Schedule (Seconds (3.0), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 3, true);
  Simulator::Schedule (Seconds (3.050), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 4, false);
  // unregister TxProfile or release channel access
  Simulator::Schedule (Seconds (4.0),&MmWaveWaveNetDevice::DeleteTxProfile, sender,SCH1);
  Simulator::Schedule (Seconds (4.0),&MmWaveWaveNetDevice::StopSch, sender,SCH1);
  Simulator::Schedule (Seconds (4.0),&MmWaveWaveNetDevice::StopSch, receiver, SCH1);
  // these packets will be dropped again because of no channel access assigned and no tx profile registered
  Simulator::Schedule (Seconds (5.0), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 5, true);
  Simulator::Schedule (Seconds (5.050), &MmWaveWaveNetDeviceExample::SendIpPacket, this, 6, false);
...
bool
MmWaveWaveNetDeviceExample::ReceiveVsa (Ptr<const Packet> pkt,const Address & address, uint32_t, uint32_t)
{
...
void
MmWaveWaveNetDeviceExample::SendWsaExample ()
{
  CreateWaveNodes ();
  Ptr<MmWaveWaveNetDevice>  sender = DynamicCast<MmWaveWaveNetDevice> (devices.Get (0));
  Ptr<MmWaveWaveNetDevice>  receiver = DynamicCast<MmWaveWaveNetDevice> (devices.Get (1));
...
// of VsaInfo is 100 per 5s, the VSA frame will be sent repeatedly.
  ...
  Simulator::Schedule (Seconds (1.0), &MmWaveWaveNetDevice::StartVsa, sender, vsaInfo);
  Simulator::Schedule (Seconds (3.0), &MmWaveWaveNetDevice::StopVsa, sender, SCH1);
...
// these WSA packets cannot be transmitted because of no channel access assigned
  Simulator::Schedule (Seconds (5.0), &MmWaveWaveNetDevice::StartVsa, sender, vsaInfo);
  Simulator::Schedule (Seconds (6.0), &MmWaveWaveNetDevice::StopVsa, sender, SCH1);
...
main (int argc, char *argv[])
{
  ...
  MmWaveWaveNetDeviceExample example;
